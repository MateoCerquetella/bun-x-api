...
type TResponseRejecter = (
  error: ApiResponseError | ApiPartialResponseError
) => void;
...
export class RequestHandlerHelper<T> {
  ...
  protected isCompressionDisabled() {
    return (
      !this.requestData.compression ||
      this.requestData.compression === 'identity'
    );
  }

  protected isFormEncodedEndpoint() {
    return this.requestData.url.href.startsWith(
  ...
  protected createPartialResponseError(
    error: Error,
    abortClose: boolean
  ...
  protected createResponseError({
    ...
    rateLimit,
    code
  }: IBuildErrorParams): ApiResponseError {
    if (TwitterApiV2Settings.debug) {
      TwitterApiV2Settings.logger.log(
        `Request failed with code ${code}, data:`,
        data
      );
  ...
  protected detectResponseType(res: IncomingMessage): TResponseParseMode {
    ...
    if (
      ...
      res.headers['content-type']?.includes('application/problem+json')
    ) {
      return 'json';
    }
  ...
  protected getParsedResponse(res: IncomingMessage) {
    ...
    const mode =
      this.requestData.forceParseMode || this.detectResponseType(res);
    ...
    if (!this.requestErrorHandled) {
      return reject(
        this.createRequestError(
          new Error('Socket closed without any information.')
        )
      );
    }
  ...
  protected requestErrorHandler(reject: TRequestRejecter, requestError: Error) {
    this.requestData.requestEventDebugHandler?.('request-error', {
      requestError
    });
  ...
  protected classicResponseHandler(
    resolve: TResponseResolver<T>,
    reject: TResponseRejecter,
    res: IncomingMessage
  ) {
    this.res = res;
    ...
    if (this.requestData.requestEventDebugHandler) {
      ...
      res.on('aborted', (error) =>
        this.requestData.requestEventDebugHandler!('response-aborted', {
          error
        })
      );
      res.on('error', (error) =>
        this.requestData.requestEventDebugHandler!('response-error', { error })
      );
      res.on('close', () =>
        this.requestData.requestEventDebugHandler!('response-close', {
          data: this.responseData
        })
      );
      res.on('end', () =>
        this.requestData.requestEventDebugHandler!('response-end')
      );
    }
  }

  protected onResponseEndHandler(
    resolve: TResponseResolver<T>,
    reject: TResponseRejecter
  ) {
    ...
    if (code >= 400) {
      reject(
        this.createResponseError({ data, res: this.res, rateLimit, code })
      );
      return;
    }
    ...
    if (TwitterApiV2Settings.debug) {
      TwitterApiV2Settings.logger.log(
        `[${this.requestData.options.method} ${this.hrefPathname}]: Request succeeds with code ${this.res.statusCode}`
      );
  ...
  protected onResponseCloseHandler(
    resolve: TResponseResolver<T>,
    reject: TResponseRejecter
  ) {
    const res = this.res;
    ...
    if (!res.complete) {
      return reject(
        this.createPartialResponseError(
          new Error(
            'Response has been interrupted before response could be parsed.'
          ),
          true
        )
      );
  ...
  protected streamResponseHandler(
    resolve: TReadyRequestResolver,
    reject: TResponseRejecter,
    res: IncomingMessage
  ) {
    ...
    if (code < 400) {
      if (TwitterApiV2Settings.debug) {
        TwitterApiV2Settings.logger.log(
          `[${this.requestData.options.method} ${this.hrefPathname}]: Request succeeds with code ${res.statusCode} (starting stream)`
        );
      }
  ...
  protected debugRequest() {
    ...
    TwitterApiV2Settings.logger.log(
      `[${this.requestData.options.method} ${this.hrefPathname}]`,
  ...
  protected buildRequest() {
    ...
    if (
      this.requestData.compression === true ||
      this.requestData.compression === 'brotli'
    ) {
    ...
    this.req = request({
      ...this.requestData.options,
      // Define URL params manually, addresses dependencies error https://github.com/PLhery/node-bun-x-api/issues/94
      host: url.hostname,
  ...
  protected registerRequestEventDebugHandlers(req: ClientRequest) {
    ...
    req.on('socket', (socket) => {
      ...
      socket.on('error', (error) =>
        this.requestData.requestEventDebugHandler!('socket-error', {
          socket,
          error
        })
      );
      socket.on('connect', () =>
        this.requestData.requestEventDebugHandler!('socket-connect', { socket })
      );
      socket.on('close', (withError) =>
        this.requestData.requestEventDebugHandler!('socket-close', {
          socket,
          withError
        })
      );
      socket.on('end', () =>
        this.requestData.requestEventDebugHandler!('socket-end', { socket })
      );
      socket.on('lookup', (...data) =>
        this.requestData.requestEventDebugHandler!('socket-lookup', {
          socket,
          data
        })
      );
      socket.on('timeout', () =>
        this.requestData.requestEventDebugHandler!('socket-timeout', { socket })
      );
    });
  }
  ...
  makeRequest() {
    ...
    return new Promise<TwitterResponse<T>>((resolve, reject) => {
      ...
      req.on(
        'response',
        this.classicResponseHandler.bind(this, resolve, reject)
      );
  ...
  async makeRequestAsStream() {
    const { req, res, requestData, originalResponse } =
      await this.makeRequestAndResolveWhenReady();
    return new TweetStream<T>(requestData as TRequestFullStreamData, {
      req,
      res,
      originalResponse
    });
  ...
  makeRequestAndResolveWhenReady() {
    ...
    return new Promise<TRequestReadyPayload>((resolve, reject) => {
      ...
      req.on(
        'response',
        this.streamResponseHandler.bind(this, resolve, reject)
      );
