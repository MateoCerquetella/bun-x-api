...
export function isTweetStreamV2ErrorPayload(payload: any) {
  ...
  return (
    typeof payload === 'object' && 'errors' in payload && !('data' in payload)
  );
}
...
export function safeDeprecationWarning(message: IDeprecationWarning) {
  if (
    ...
    !console.warn ||
    !TwitterApiV2Settings.deprecationWarnings
  ) {
    return;
  }
  ...
  const formattedMsg =
    `[bun-x-api] Deprecation warning: In ${message.instance}.${message.method}() call` +
    `, ${message.problem}.\n${message.resolution}.`;

  console.warn(formattedMsg);
  console.warn(
    'To disable this message, import variable TwitterApiV2Settings from bun-x-api and set TwitterApiV2Settings.deprecationWarnings to false.'
  );
