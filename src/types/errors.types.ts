...
abstract class ApiError extends Error {
  abstract type:
    | ETwitterApiError.Request
    | ETwitterApiError.Response
    | ETwitterApiError.PartialResponse;
...
export class ApiRequestError
  extends ApiError
  implements TwitterApiRequestError
{
...
export class ApiPartialResponseError
  extends ApiError
  implements IBuildApiPartialRequestError
{
  protected _options: any;
...
export class ApiResponseError
  extends ApiError
  implements TwitterApiError, IBuildApiResponseError
{
  protected _options: any;
  ...
  constructor(message: string, options: IBuildApiResponseError) {
    ...
    // Fix bad error data payload on some v1 endpoints (see https://github.com/PLhery/node-bun-x-api/issues/342)
    if (
      options.data &&
      typeof options.data === 'object' &&
      'error' in options.data &&
      !options.data.errors
    ) {
  ...
  hasErrorCode(
    ...codes: (EApiV1ErrorCode | number)[] | (EApiV2ErrorCode | string)[]
  ) {
